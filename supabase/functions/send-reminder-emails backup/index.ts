...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: subscriptions, error: subscriptionError } = await supabase.from("email_subscriptions").select("*").eq("is_active", true);
    ...
    const users = usersData.users.filter((u)=>userIds.includes(u.id) && u.email).map((u)=>({
        id: u.id,
        email: u.email
      })) // We filtered for users with email
    ;
    ...
    for (const subscription of subscriptions){
      ...
      if (shouldSend) {
        ...
        let { data: entries, error: entriesError } = await supabase.from("entries")// Select new fields for AI content and images
        .select("id, title, content, date, mood, ai_summary, positive_affirmation, image_urls").eq("user_id", user.id)// .order("date", { ascending: false }) // Maybe order by random()? Check Supabase docs
        ...
        if (entries && entries.length > 0) {
          ...
          let affirmationsForEmail = [];
          let aiSummaryForEmail = randomEntry.ai_summary;
          if (!randomEntry.ai_summary || !randomEntry.positive_affirmation || Array.isArray(randomEntry.positive_affirmation) && randomEntry.positive_affirmation.length === 0 || randomEntry.positive_affirmation === "Could not generate affirmations.") {
            ...
            try {
              const { data: aiData, error: aiError } = await supabase.functions.invoke("generate-ai-content", {
                body: {
                  entryContent: randomEntry.content
                }
              });
              if (aiError) {
                ...
                if (typeof aiData.affirmations === 'string') {
                  try {
                    ...
                    if (Array.isArray(parsedAffirmations)) {
                      affirmationsForEmail = parsedAffirmations.filter((item)=>typeof item === 'object' && item !== null && 'text' in item);
                    } else {
                    // affirmationsForEmail remains empty, string will be handled below
                    }
                  } catch (e) {
                  ...
                  affirmationsForEmail = aiData.affirmations.filter((item)=>typeof item === 'object' && item !== null && 'text' in item);
                }
            ...
            if (typeof randomEntry.positive_affirmation === 'string') {
              try {
                ...
                if (Array.isArray(parsed)) {
                  affirmationsForEmail = parsed.filter((item)=>typeof item === 'object' && item !== null && 'text' in item);
                }
              } catch (e) {}
            } else if (Array.isArray(randomEntry.positive_affirmation)) {
              affirmationsForEmail = randomEntry.positive_affirmation.filter((item)=>typeof item === 'object' && item !== null && 'text' in item);
            }
          }
          ...
          const currentMood = randomEntry.mood && moodMap[randomEntry.mood] ? moodMap[randomEntry.mood] : {
            emoji: "❓",
            description: "Unknown"
          };
          let moodHtml = "";
          ...
          const { error: updateError } = await supabase.from("email_subscriptions").update({
